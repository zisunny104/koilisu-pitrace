...
- **去背保留筆觸**：以 LAB 色差（ΔE76）取代單純 RGB 距離，搭配局部背景估算（同一張紙不同區域的泛黃/光影不均都會納入計算）與連通元件孤立度分析（壓低散落的雜點，不誤刪破碎/飛白筆觸），並用可調的臨界值／柔化區間平滑過渡，避免二值化吃掉淡筆觸
- **原始／遮罩／疊加／結果四種預覽模式**：原始顯示未經任何調整的掃描真實顏色，遮罩以灰階視覺化去背 alpha，疊加在原圖上標示保留範圍，結果為最終合成輸出——對比度/亮度調整只影響遮罩判定範圍，不會滲入結果的顏色
